        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 gap-4 mb-8 sm:grid-cols-3">
                ...
                <div class="p-4 bg-white border border-gray-100 rounded-lg shadow-sm">
                    ...
                    <p class="text-2xl font-bold text-indigo-600">
                        {{-- Note: sum pada paginated collection hanya menghitung halaman aktif.
                             Untuk total akurat, sebaiknya dipassing dari controller, tapi untuk UI sederhana ini ok --}}
                ...
                <div class="p-4 bg-white border border-gray-100 rounded-lg shadow-sm">
                    ...
                    {{-- Menggunakan withCount dari controller --}}
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                @forelse ($shelters as $shelter)
                    @php
                        // Logika Visual Bar
                        // Gunakan sheep_count dari withCount controller agar lebih efisien
                        $currentCount = $shelter->sheep_count;
                        $percentage = ($shelter->capacity > 0) ? ($currentCount / $shelter->capacity) * 100 : 0;

                        $colorClass = 'bg-green-500'; // Default Hijau (Aman)
                        $statusText = 'Tersedia';
                        $statusBg = 'bg-green-100 text-green-800';

                        if ($percentage >= 100) {
                            $colorClass = 'bg-red-600'; // Penuh (Merah)
                            $statusText = 'Penuh';
                            $statusBg = 'bg-red-100 text-red-800';
                        } elseif ($percentage >= 80) {
                            $colorClass = 'bg-yellow-500'; // Hampir Penuh (Kuning)
                ...
                @empty
                    <div class="col-span-1 py-12 text-center border-2 border-gray-300 border-dashed rounded-lg md:col-span-2 lg:col-span-3 bg-gray-50">
                        ...
                        <p class="text-lg text-gray-500">
                            {{ request('search') ? 'Tidak ada kandang yang cocok dengan pencarian.' : 'Belum ada kandang yang dibuat.' }}
                        </p>
