                    ...
                    <div class="grid items-end grid-cols-1 gap-4 md:grid-cols-4">
                        ...
                        <div>
                            ...
                            <select name="shelter_id" id="shelter_id" class="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                {{-- Pastikan controller mengirimkan variabel $shelters --}}
                                @if(isset($shelters))
                        ...
                        <a href="{{ route('feeding-records.show', $record) }}" class="flex-grow block">
                            ...
                            <div class="p-5">
                                <div class="grid h-full grid-cols-2 gap-4">
                                    ...
                                    {{-- PERBAIKAN: Inisialisasi variabel total biaya DI SINI --}}
                                        ...
                                        <ul class="space-y-2">
                                            ...
                                            @foreach($record->feedTypes as $feed)
                                                @if($feed->pivot->quantity_morning > 0)
                                                    @php
                                                        $hasMorning = true;
                                                        // Hitung biaya (qty pagi * harga) dan tambahkan ke total
                                                    ...
                                                    <li class="flex items-start justify-between text-sm text-gray-700">
                                                        ...
                                                        <span class="font-bold">{{ (float)$feed->pivot->quantity_morning }} <span class="text-[10px] font-normal text-gray-400">{{ $feed->unit }}</span></span>
                                                    </li>
                                                @endif
                                            @endforeach
                                            @if(!$hasMorning)
                                                <li class="py-2 text-xs italic text-center text-gray-400">- Tidak ada pakan -</li>
                                            @endif
                                        </ul>
                                    </div>
                                        ...
                                        <ul class="space-y-2">
                                            ...
                                            @foreach($record->feedTypes as $feed)
                                                @if($feed->pivot->quantity_evening > 0)
                                                    @php
                                                        $hasEvening = true;
                                                        // Hitung biaya (qty sore * harga) dan tambahkan ke total
                                                        $totalCostPerCard += ($feed->pivot->quantity_evening * ($feed->price_per_unit ?? 0));
                                                    @endphp
                                                    <li class="flex items-start justify-between text-sm text-gray-700">
                                                        <span class="w-16 text-xs text-gray-500 truncate" title="{{ $feed->name }}">{{ $feed->name }}</span>
                                                        <span class="font-bold">{{ (float)$feed->pivot->quantity_evening }} <span class="text-[10px] font-normal text-gray-400">{{ $feed->unit }}</span></span>
                                                    </li>
                                                @endif
                                            @endforeach
                                            @if(!$hasEvening)
                ...
                @empty
                    <div class="col-span-1 md:col-span-2 xl:col-span-3">
                        <div class="p-12 text-center bg-white border-2 border-gray-200 border-dashed shadow-sm rounded-xl">
                            ...
                            <h3 class="text-lg font-medium text-gray-900">Belum ada riwayat pakan</h3>
                            <p class="mt-1 text-sm text-gray-500">Mulai catat pemberian pakan harian untuk memantau nutrisi ternak.</p>
                            <div class="mt-6">
                                <a href="{{ route('feeding-records.create') }}">
