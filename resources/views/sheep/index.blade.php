                ...
                @forelse ($sheep as $s)
                    ...
                    <div class="flex flex-col h-full overflow-hidden transition-transform transform bg-white shadow-sm hover:scale-105 sm:rounded-lg {{ $cardBorderClass }}">
                        ...
                        <div class="relative block group">
                            {{-- Jika rejected, link disable atau arahkan ke edit --}}
                            <a href="{{ route('sheep.show', $s) }}" class="block">
                                <div class="relative">
                                    <img
                                        class="object-cover w-full h-48 {{ $isRejected ? 'opacity-50 grayscale' : '' }}"
                                    ...
                                    {{-- OVERLAY JIKA DITOLAK (DENGAN TOMBOL AKSI) --}}
                                    @if($isRejected)
                                        <div class="absolute inset-0 flex flex-col items-center justify-center p-4 text-center bg-black/60">
                                            <div class="mb-2 text-white">
                                                <svg class="w-8 h-8 mx-auto mb-1 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                <span class="text-sm font-bold text-red-400">PERMINTAAN DITOLAK</span>
                                                <p class="mt-1 text-xs italic text-gray-300 line-clamp-2">"{{ $rejectionNote }}"</p>
                                            </div>

                                            <div class="flex w-full gap-2 mt-2">
                                                {{-- Tombol Coba Lagi (Edit) --}}
                                                <a href="{{ route('sheep.edit', $s) }}" class="flex-1 px-2 py-1 text-xs font-bold text-center text-white bg-indigo-600 rounded hover:bg-indigo-700">
                                                    Coba Lagi
                                                </a>
                                                ...
                                                <form action="{{ route('sheep.destroy', $s) }}" method="POST" onsubmit="return confirm('Hapus data domba {{ $s->tag_number }} permanen?');" class="flex-1">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="w-full px-2 py-1 text-xs font-bold text-white bg-red-600 rounded hover:bg-red-700">
                                                        Hapus
                                                    </button>
                                                </form>
                                            </div>
                                    ...
                                    {{-- Sembunyikan badge lain jika rejected agar tidak semrawut --}}
                                    @if(!$isRejected)
                                        <div class="absolute flex flex-col items-end gap-1 top-2 right-2">
                                            @if($s->placement_status === 'Partner')
                                                ...
                                                @else
                                                    {{-- Pending Request --}}
                                                    <span class="flex items-center gap-1 bg-yellow-50 text-yellow-700 border border-yellow-200 text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">
                                ...
                                <div class="p-4 {{ $isRejected ? 'opacity-50' : '' }}">
                        ...
                        {{-- FOOTER KARTU: QUICK ACTIONS (Disable jika Rejected agar fokus ke overlay) --}}
                        @if(!$isRejected)
